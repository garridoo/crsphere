...
tecan.file <- paste(results.dir, "fluorescence_SC_split_stericup.txt", sep="")
...
dat <- cast(tecan, condition ~ Sample, value="Value")
rownames(dat) <- dat[, 1]
...
design <- unique(data.frame(Sample=tecan$Sample, split_exp=tecan$split_exp, replicate=tecan$replicate))
...
p <- ggplot(df, aes(x=condition, y=value, color=condition, shape=condition)) +
            ...
            labs(x="Condition", y="Fluorescence") +
            ggtitle("Fluorescence SynCom split experiment") +
            main_theme +
            theme(legend.position="none")

ggsave(paste(figures.dir, "fluorescence_SC_split.pdf", sep=""), p, width=12, height=5)
