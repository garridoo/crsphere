...
design.file <- paste(data.dir, "design_SC_split_stericup.txt", sep="")
# design.file <- paste(data.dir, "design_SC_split_bottle.txt", sep="")
asv_table.file <- paste(results.dir, "asv_table_SC_split.txt", sep="")
...
# add fields to design table

design$condition <- paste(design$Content, design$Partner, sep="/")
design$exp_cond <- paste(design$split_exp, design$condition, sep="_")

design$chlamy <- "C"
design$chlamy[design$condition %in% c("SC/TP")] <- "SC"
design$chlamy[design$condition %in% c("Input/")] <- "Input"
