...
design.file <- paste(data.dir, "design_NC.txt", sep="")
asv_table.file <- paste(results.dir, "asv_table_MC.txt", sep="")
taxonomy.file <- paste(results.dir, "taxonomy_MC.txt", sep="")
...
design$AP <- FALSE
design$AP[grepl("AP", design$medium)] <- TRUE

design$community <- as.character(design$community)
design$community[design$medium=="Input"] <- "Input"

design$community_AP <- paste(design$community, design$AP, sep="_")
